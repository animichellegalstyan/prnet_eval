...
if __name__ == "__main__":
    ...
    epochs = 1 if args_train.smoke_test else 500
    save_directory = './checkpoint_test/' if args_train.smoke_test else './checkpoint/'
    ...
    folds_to_run = [0] if args_train.smoke_test else range(5)

    for fold in folds_to_run:

        current_split_key = f"{args_train.split_key}_split_{fold}"

        current_save_dir = os.path.join(save_directory, f"fold_{fold}")
        os.makedirs(current_save_dir, exist_ok=True)

        print(f"\n==========================================")
        print(f" STARTING TRAINING FOR FOLD {fold}")
        print(f" Using split_key: {current_split_key}")
        print(f" Saving checkpoints to: {current_save_dir}")
        print(f"==========================================\n")
