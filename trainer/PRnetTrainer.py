...
class PRnetTrainer:
    ...
    # get_device and safe_softplus are helper-functions that aid in making the code compatible with cuda and mps
    def get_device(self):
        if torch.cuda.is_available():
            return torch.device("cuda")       # NVIDIA GPU
        elif torch.backends.mps.is_available():
            return torch.device("mps")        # Apple Silicon (M1, M2, M3, M5)
        else:
    ...
    def __init__(self, adata, batch_size = 32, comb_num = 2, shuffle = True, split_key='random_split', model_save_dir = './checkpoint/',results_save_dir = './results/', x_dimension = 5000, hidden_layer_sizes = [128], z_dimension = 64, adaptor_layer_sizes = [128], comb_dimension = 64, drug_dimension = 1031, n_genes=20,  dr_rate = 0.05, loss = ['GUSS'], obs_key = 'cov_drug_name', **kwargs): # maybe add more parameters
        ...
        # build in other options for when cuda is not available
    ...
    def test(self, model_path, return_dict = False):
        ...
        self.modelPGM.load_state_dict(torch.load(model_path, map_location=self.device))
        self.modelPGM.eval()
        ...
        for j, vdata in loop_t:
            ...
            #print(f"control: {control.dtype}, target: {target.dtype}, encode_label: {encode_label.dtype}, noise: {noise.dtype}")
